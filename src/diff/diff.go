...
import (
	...
	"github.com/elee1766/gofer/src/tui/theme"
...
func SyntaxHighlight(w io.Writer, source, fileName, formatter string, bg lipgloss.TerminalColor, t theme.Theme) error {
...
func highlightLine(fileName string, line string, bg lipgloss.TerminalColor, t theme.Theme) string {
	var buf bytes.Buffer
	err := SyntaxHighlight(&buf, line, fileName, "terminal16m", bg, t)
	if err != nil {
		return line
	}
	return buf.String()
...
func applyHighlighting(content string, segments []Segment, segmentType LineType, highlightBg lipgloss.AdaptiveColor, t theme.Theme) string {
	...
	fgColor := lipgloss.Color(getColor(t.Background()))
...
func renderLeftColumn(fileName string, dl *DiffLine, colWidth int, t theme.Theme) string {
	...
	content := highlightLine(fileName, dl.Content, bgStyle.GetBackground(), t)

	// Apply intra-line highlighting for removed lines
	if dl.Kind == LineRemoved && len(dl.Segments) > 0 {
		content = applyHighlighting(content, dl.Segments, LineRemoved, t.DiffHighlightRemoved(), t)
	}
...
func renderRightColumn(fileName string, dl *DiffLine, colWidth int, t theme.Theme) string {
	...
	if dl.Kind == LineAdded && len(dl.Segments) > 0 {
		content = applyHighlighting(content, dl.Segments, LineAdded, t.DiffHighlightAdded(), t)
	}
...
func RenderSideBySideHunk(fileName string, h Hunk, t theme.Theme, opts ...SideBySideOption) string {
	...
	var sb strings.Builder
	for _, p := range pairs {
		leftStr := renderLeftColumn(fileName, p.left, leftWidth, t)
		rightStr := renderRightColumn(fileName, p.right, rightWidth, t)
...
func FormatDiff(diffText string, t theme.Theme, opts ...SideBySideOption) (string, error) {
	...
	for _, h := range diffResult.Hunks {
		sb.WriteString(RenderSideBySideHunk(diffResult.OldFile, h, t, opts...))
	}
